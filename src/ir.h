...
struct Operand {
    ...
    bool isLocalVar() const { return type == OperandType::LOCAL_VAR; }
    bool isGlobalVar() const { return type == OperandType::GLOBAL_VAR; }
    bool isLabel() const { return type == OperandType::LABEL; }
    bool isFunc() const { return type == OperandType::FUNC; }
...
struct IRInst {
    ...
    IRInst() : op(IROp::ADD), dest(Operand::none()),
               src1(Operand::none()), src2(Operand::none()) {}

    IRInst(IROp o, Operand d, Operand s1, Operand s2)
        : op(o), dest(std::move(d)), src1(std::move(s1)), src2(std::move(s2)) {}
};
...
inline const char* irOpToString(IROp op) {
    switch (op) {
        ...
        case IROp::GLOBAL_VAR_DECL: return "GLOBAL_VAR_DECL";
        case IROp::LOCAL_VAR_DECL:  return "LOCAL_VAR_DECL";
    }
    return "UNKNOWN";
}
...
inline std::string operandToString(const Operand& op) {
    switch (op.type) {
        case OperandType::IMM:        return "#" + std::to_string(op.immVal);
        case OperandType::LOCAL_VAR:  return "%" + op.name;
        case OperandType::GLOBAL_VAR: return "@" + op.name;
        case OperandType::LABEL:      return op.name + ":";
        case OperandType::FUNC:       return op.name;
        case OperandType::PARAM:      return "$" + op.name;
        case OperandType::NONE:       return "_";
    }
    return "?";
}
