    ...
    for (auto& node : compUnit.globalDecls) {
        if (auto* funcDef = dynamic_cast<FuncDef*>(node.get())) {
            ...
            if (symTable.lookupFunction(funcDef->name)) {
                error(funcDef->line, funcDef->col,
                      "function '" + funcDef->name + "' already defined");
                continue;
            }
...
Type SemanticAnalyzer::analyzeExpr(Expr* expr) {
    ...
    if (auto* binaryExpr = dynamic_cast<BinaryExpr*>(expr)) {
        ...
        if (binaryExpr->op == BinOp::ASSIGN) {
            ...
            if (auto* varExpr = dynamic_cast<VarExpr*>(binaryExpr->lhs.get())) {
                ...
                error(binaryExpr->lhs->line, binaryExpr->lhs->col,
        ...
        if (lhsType == Type::VOID || rhsType == Type::VOID) {
            error(binaryExpr->line, binaryExpr->col,
                  "cannot perform binary operation on void type");
        }
    ...
    if (auto* unaryExpr = dynamic_cast<UnaryExpr*>(expr)) {
        ...
        if (operandType == Type::VOID) {
            error(unaryExpr->line, unaryExpr->col,
    ...
    if (auto* callExpr = dynamic_cast<CallExpr*>(expr)) {
        ...
        if (!funcSym) {
            error(callExpr->line, callExpr->col,
                  "undefined function '" + callExpr->funcName + "'");
            expr->type = Type::INT;
            return Type::INT;
        }
        ...
        if (callExpr->args.size() != funcSym->paramTypes.size()) {
            error(callExpr->line, callExpr->col,
                  ...
                  std::to_string(funcSym->paramTypes.size()) +
                  " arguments, but " + std::to_string(callExpr->args.size()) +
                  " were given");
        }
...
bool SemanticAnalyzer::isConstExpr(Expr* expr) {
    ...
    if (auto* binaryExpr = dynamic_cast<BinaryExpr*>(expr)) {
        return isConstExpr(binaryExpr->lhs.get()) &&
               isConstExpr(binaryExpr->rhs.get());
    }
...
bool SemanticAnalyzer::tryConstFold(Expr* expr, int& result) {
    ...
    if (auto* binaryExpr = dynamic_cast<BinaryExpr*>(expr)) {
        ...
        if (!tryConstFold(binaryExpr->lhs.get(), lhsVal)) return false;
        if (!tryConstFold(binaryExpr->rhs.get(), rhsVal)) return false;

        switch (binaryExpr->op) {
            case BinOp::ADD: result = lhsVal + rhsVal; break;
            case BinOp::SUB: result = lhsVal - rhsVal; break;
            case BinOp::MUL: result = lhsVal * rhsVal; break;
            case BinOp::DIV:
                if (rhsVal == 0) return false;  // 除以零
            ...
            case BinOp::MOD:
                ...
                result = lhsVal % rhsVal;
                break;
            case BinOp::LT: result = (lhsVal < rhsVal) ? 1 : 0; break;
            case BinOp::GT: result = (lhsVal > rhsVal) ? 1 : 0; break;
            case BinOp::LE: result = (lhsVal <= rhsVal) ? 1 : 0; break;
            case BinOp::GE: result = (lhsVal >= rhsVal) ? 1 : 0; break;
            case BinOp::EQ: result = (lhsVal == rhsVal) ? 1 : 0; break;
            case BinOp::NE: result = (lhsVal != rhsVal) ? 1 : 0; break;
            case BinOp::AND: result = (lhsVal && rhsVal) ? 1 : 0; break;
            case BinOp::OR:  result = (lhsVal || rhsVal) ? 1 : 0; break;
    ...
    if (auto* unaryExpr = dynamic_cast<UnaryExpr*>(expr)) {
        int operandVal = 0;
        if (!tryConstFold(unaryExpr->operand.get(), operandVal)) return false;

        switch (unaryExpr->op) {
            case UnaryOp::POS: result = operandVal; break;
            case UnaryOp::NEG: result = -operandVal; break;
            case UnaryOp::NOT: result = (operandVal == 0) ? 1 : 0; break;
        }
        return true;
    }
