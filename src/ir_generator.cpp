...
void IRGenerator::genFuncDef(FuncDef* funcDef) {
    ...
    for (auto& param : funcDef->params) {
        emit(IROp::LOCAL_VAR_DECL,
             Operand::localVar(param->name),
             Operand::param(param->name));
    }
...
void IRGenerator::genDecl(Decl* decl) {
    ...
    if (isGlobalContext) {
        // 全局变量
        int initVal = 0;
        emit(IROp::GLOBAL_VAR_DECL,
             Operand::globalVar(decl->name),
             Operand::imm(initVal));
        if (decl->initExpr) {
...
Operand IRGenerator::genExpr(Expr* expr) {
    ...
    if (auto* binaryExpr = dynamic_cast<BinaryExpr*>(expr)) {
        ...
        switch (binaryExpr->op) {
            case BinOp::ADD: irOp = IROp::ADD; break;
            case BinOp::SUB: irOp = IROp::SUB; break;
            case BinOp::MUL: irOp = IROp::MUL; break;
            case BinOp::DIV: irOp = IROp::DIV; break;
            case BinOp::MOD: irOp = IROp::MOD; break;
            case BinOp::LT:  irOp = IROp::LT;  break;
            case BinOp::GT:  irOp = IROp::GT;  break;
            case BinOp::LE:  irOp = IROp::LE;  break;
            case BinOp::GE:  irOp = IROp::GE;  break;
            case BinOp::EQ:  irOp = IROp::EQ;  break;
            case BinOp::NE:  irOp = IROp::NE;  break;
            default: irOp = IROp::ADD; break;
        }
    ...
    if (auto* unaryExpr = dynamic_cast<UnaryExpr*>(expr)) {
        ...
        IROp irOp;
        switch (unaryExpr->op) {
            case UnaryOp::POS: irOp = IROp::ASSIGN; break;  // 正号直接赋值
            case UnaryOp::NEG: irOp = IROp::SUB; break;     // 取负 = 0 - x
            case UnaryOp::NOT: irOp = IROp::NOT; break;
        }
