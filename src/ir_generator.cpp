...
void IRGenerator::optimizePass() {
  ...
  // 循环不变量外提（LICM），破坏语义。
  {
    ...
    // 证明循环体确实只有一个这种模累加器，并枚举至多 65536 个余数相位；仅当
    // 初值和每轮增量被证明始终同号时，才用闭式前缀和汇总整个循环。可能跨 0
    // 的递推以及含控制流、调用、全局写、溢出风险、多个可观察状态或不兼容模数
    // 的循环均保守回退，不在编译期逐轮解释源循环。
    {
      ...
      for (int summaryRound = 0; summaryRound < 8; ++summaryRound) {
        ...
        for (std::size_t loopStart = ir.size(); loopStart-- > 0 && !summarized;) {
          ...
          // 对所有迭代使用区间上界证明增量不会变号。这样重复的 C 有符号余数
          // 与对闭式总和取一次余数等价；证明不了时保留原循环。
          const WideInt firstAffine =
              static_cast<WideInt>(recurrence.induction) * *initialInduction;
          const WideInt lastAffine =
              static_cast<WideInt>(recurrence.induction) * (finalInduction - 1);
          const WideInt minimumDelta = std::min(firstAffine, lastAffine) + *minimumPhase;
          const WideInt maximumDelta = std::max(firstAffine, lastAffine) + *maximumPhase;
          const bool nonnegativeRecurrence = *initialAccumulator >= 0 && minimumDelta >= 0;
          const bool nonpositiveRecurrence = *initialAccumulator <= 0 && maximumDelta <= 0;
          if (!nonnegativeRecurrence && !nonpositiveRecurrence) {
            continue;
          }
          const WideInt total = static_cast<WideInt>(*initialAccumulator) +
                                sumDeltas(*initialInduction, static_cast<std::uint64_t>(trips));
          const std::int64_t accumulator =
              static_cast<std::int64_t>(truncRemainder(total, *accumulatorModulus));
