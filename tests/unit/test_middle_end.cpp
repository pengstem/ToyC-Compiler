...
static std::shared_ptr<FuncDef> makeFuncDef(Type retType, const std::string& name,
...
static std::shared_ptr<BinaryExpr> makeBinary(BinOp op, std::shared_ptr<Expr> lhs,
                                              std::shared_ptr<Expr> rhs) {
  return std::make_shared<BinaryExpr>(op, std::move(lhs), std::move(rhs));
}

static std::shared_ptr<CallExpr> makeCall(const std::string& name,
                                          std::vector<std::shared_ptr<Expr>> args) {
  return std::make_shared<CallExpr>(name, std::move(args));
}

static std::shared_ptr<Decl> makeDecl(bool isConst, const std::string& name,
...
static std::shared_ptr<IfStmt> makeIf(std::shared_ptr<Expr> cond, std::shared_ptr<Stmt> thenBranch,
...
static std::shared_ptr<WhileStmt> makeWhile(std::shared_ptr<Expr> cond,
...
void testSema_basicMain() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {}, makeBlock({makeReturn(makeInt(0))})));
...
void testSema_voidReturnValue() {
  ...
  auto comp = makeCompUnit();
  comp->globalDecls.push_back(
      makeFuncDef(Type::VOID, "f", {}, makeBlock({makeReturn(makeInt(42))})));
  SemanticAnalyzer sema;
...
void testSema_nonVoidMustReturn() {
  ...
  auto comp = makeCompUnit();
  comp->globalDecls.push_back(makeFuncDef(Type::INT, "main", {}, makeBlock({makeReturn()})));
  SemanticAnalyzer sema;
  bool ok = sema.analyze(*comp);
...
void testSema_undefinedVar() {
  ...
  auto comp = makeCompUnit();
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {}, makeBlock({makeReturn(makeVar("x"))})));
  SemanticAnalyzer sema;
  bool ok = sema.analyze(*comp);
  if (!ok && !sema.getErrors().empty()) {
    testPass();
...
void testSema_constFold() {
  ...
  auto expr =
      makeBinary(BinOp::SUB,
                 makeBinary(BinOp::ADD, makeInt(1), makeBinary(BinOp::MUL, makeInt(2), makeInt(3))),
                 makeInt(4));
  SemanticAnalyzer sema;
  int result = 0;
...
void testSema_breakOutsideLoop() {
  ...
  auto comp = makeCompUnit();
  comp->globalDecls.push_back(makeFuncDef(Type::INT, "main", {}, makeBlock({makeBreak()})));
  SemanticAnalyzer sema;
  bool ok = sema.analyze(*comp);
  if (!ok && !sema.getErrors().empty()) {
    testPass();
...
void testSema_assignToConst() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {}, makeBlock({makeReturn(makeVar("c"))})));
  SemanticAnalyzer sema;
  if (sema.analyze(*comp)) {
    testPass();
  } else {
...
void testIR_simpleReturn() {
  ...
  if (irContains(ir, "FUNC_BEGIN main") && irContains(ir, "RETURN #42") &&
...
void testIR_shortCircuitOr() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeReturn(makeBinary(BinOp::OR, makeInt(1), makeInt(0)))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_ifStatement() {
  ...
  comp->globalDecls.push_back(makeFuncDef(
      Type::INT, "main", {},
      makeBlock({makeIf(makeInt(1), makeReturn(makeInt(100))), makeReturn(makeInt(0))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_whileLoop() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeWhile(makeInt(1), makeBreak()), makeReturn(makeInt(0))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_functionCall() {
  ...
  comp->globalDecls.push_back(makeFuncDef(
      Type::INT, "main", {}, makeBlock({makeReturn(makeCall("add", {makeInt(1), makeInt(2)}))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_voidFunction() {
  ...
  auto comp = makeCompUnit();
  comp->globalDecls.push_back(makeFuncDef(Type::VOID, "f", {}, makeBlock({})));
  comp->globalDecls.push_back(makeFuncDef(
      Type::INT, "main", {}, makeBlock({makeExprStmt(makeCall("f", {})), makeReturn(makeInt(1))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_scopeVariable() {
  ...
  comp->globalDecls.push_back(makeFuncDef(
      Type::INT, "main", {},
      makeBlock({makeDeclStmt(makeDecl(false, "x", makeInt(42))), makeReturn(makeVar("x"))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testSema_unaryNeg() {
  ...
  auto comp = makeCompUnit();
  comp->globalDecls.push_back(makeFuncDef(
      Type::INT, "main", {}, makeBlock({makeReturn(makeUnary(UnaryOp::NEG, makeInt(42)))})));
  SemanticAnalyzer sema;
  if (sema.analyze(*comp)) {
    testPass();
  } else {
...
void testSema_unaryNot() {
  ...
  auto comp = makeCompUnit();
  comp->globalDecls.push_back(makeFuncDef(
      Type::INT, "main", {}, makeBlock({makeReturn(makeUnary(UnaryOp::NOT, makeInt(0)))})));
  SemanticAnalyzer sema;
  if (sema.analyze(*comp)) {
    testPass();
  } else {
...
void testSema_unaryPos() {
  ...
  auto comp = makeCompUnit();
  comp->globalDecls.push_back(makeFuncDef(
      Type::INT, "main", {}, makeBlock({makeReturn(makeUnary(UnaryOp::POS, makeInt(5)))})));
  SemanticAnalyzer sema;
  if (sema.analyze(*comp)) {
    testPass();
  } else {
...
void testIR_compareLT() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeReturn(makeBinary(BinOp::LT, makeInt(3), makeInt(5)))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_compareGT() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeReturn(makeBinary(BinOp::GT, makeInt(5), makeInt(3)))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_compareLE() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeReturn(makeBinary(BinOp::LE, makeInt(3), makeInt(3)))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_compareGE() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeReturn(makeBinary(BinOp::GE, makeInt(5), makeInt(5)))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_compareEQ() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeReturn(makeBinary(BinOp::EQ, makeInt(3), makeInt(3)))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_compareNE() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeReturn(makeBinary(BinOp::NE, makeInt(3), makeInt(5)))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testSema_constFoldLogical() {
  ...
  auto expr = makeBinary(BinOp::OR, makeInt(1), makeBinary(BinOp::AND, makeInt(0), makeInt(1)));
  SemanticAnalyzer sema;
  int result = 0;
  bool folded = sema.tryConstFold(expr.get(), result);
  if (folded && result == 1) {
...
void testSema_constFoldCompare() {
  ...
  auto expr = makeBinary(BinOp::AND, makeBinary(BinOp::GT, makeInt(5), makeInt(3)),
...
void testSema_constFoldUnary() {
  ...
  auto expr = makeUnary(UnaryOp::NEG, makeUnary(UnaryOp::NEG, makeInt(10)));
  SemanticAnalyzer sema;
  int result = 0;
  bool folded = sema.tryConstFold(expr.get(), result);
  if (folded && result == 10) {
...
void testSema_constFoldDivMod() {
  ...
  auto expr = makeBinary(
      BinOp::ADD,
      makeBinary(BinOp::MUL, makeBinary(BinOp::DIV, makeInt(10), makeInt(3)), makeInt(3)),
...
void testSema_varNameShadowFunc() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeDeclStmt(makeDecl(false, "main", makeInt(10))),
...
void testSema_multiLevelScope() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeDeclStmt(makeDecl(false, "x", makeInt(1))),
                             makeBlock({makeDeclStmt(makeDecl(false, "x", makeInt(2))),
...
void testIR_nestedIf() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeIf(makeInt(1), makeIf(makeInt(2), makeReturn(makeInt(42)))),
                             makeReturn(makeInt(0))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
...
void testIR_continueInWhile() {
  ...
  comp->globalDecls.push_back(makeFuncDef(
      Type::INT, "main", {},
      makeBlock({makeWhile(makeInt(1), makeBlock({makeContinue(), makeReturn(makeInt(0))})),
...
void testIR_whileIf() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeWhile(makeBinary(BinOp::LT, makeVar("i"), makeInt(10)),
                                       makeIf(makeInt(1), makeBreak())),
                             makeReturn(makeInt(0))})));
  // Need to declare i first
  comp->globalDecls[0] =
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeDeclStmt(makeDecl(false, "i", makeInt(0))),
                             makeWhile(makeBinary(BinOp::LT, makeVar("i"), makeInt(10)),
                                       makeIf(makeInt(1), makeBreak())),
                             makeReturn(makeInt(0))}));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
...
void testIR_nestedWhile() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeWhile(makeInt(1), makeWhile(makeInt(0), makeBreak())),
                             makeReturn(makeInt(0))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
...
void testSema_continueOutsideLoop() {
  ...
  auto comp = makeCompUnit();
  comp->globalDecls.push_back(makeFuncDef(Type::INT, "main", {}, makeBlock({makeContinue()})));
  SemanticAnalyzer sema;
  bool ok = sema.analyze(*comp);
  if (!ok && !sema.getErrors().empty()) {
    testPass();
...
void testSema_breakInNestedLoop() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeWhile(makeInt(1), makeWhile(makeInt(1), makeBreak())),
                             makeReturn(makeInt(0))})));
  SemanticAnalyzer sema;
  if (sema.analyze(*comp)) {
    testPass();
...
void testIR_multiParams() {
  ...
  comp->globalDecls.push_back(makeFuncDef(
      ...
      makeBlock({makeReturn(makeBinary(
          BinOp::ADD, makeBinary(BinOp::ADD, makeVar("a"), makeVar("b")), makeVar("c")))})));
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeReturn(makeCall("sum3", {makeInt(1), makeInt(2), makeInt(3)}))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_recursion() {
  ...
  comp->globalDecls.push_back(makeFuncDef(
      ...
      makeBlock(
          {makeIf(makeBinary(BinOp::LT, makeVar("n"), makeInt(2)), makeReturn(makeVar("n"))),
           makeReturn(makeBinary(
               BinOp::ADD, makeCall("fib", {makeBinary(BinOp::SUB, makeVar("n"), makeInt(1))}),
...
void testSema_funcParamCountMismatch() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeReturn(makeCall("add", {makeInt(1)}))}))); // 只传 1 个参数
  SemanticAnalyzer sema;
  bool ok = sema.analyze(*comp);
  if (!ok && !sema.getErrors().empty()) {
    testPass();
...
void testSema_callUndefinedFunc() {
  ...
  auto comp = makeCompUnit();
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {}, makeBlock({makeReturn(makeCall("noSuchFunc", {}))})));
  SemanticAnalyzer sema;
  bool ok = sema.analyze(*comp);
  if (!ok && !sema.getErrors().empty()) {
    testPass();
...
void testSema_assignToNonLvalue() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {},
                  makeBlock({makeExprStmt(makeBinary(BinOp::ASSIGN, makeInt(5), makeInt(10))),
                             makeReturn(makeInt(0))})));
  SemanticAnalyzer sema;
  bool ok = sema.analyze(*comp);
  if (!ok && !sema.getErrors().empty()) {
  ...
  comp->globalDecls.push_back(makeFuncDef(
      ...
      makeBlock(
          {makeDeclStmt(makeDecl(false, "a", makeInt(1))),
           ...
           makeReturn(makeBinary(BinOp::ADD, makeBinary(BinOp::ADD, makeVar("a"), makeVar("b")),
                                 makeVar("c")))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
  if (irContains(ir, "LOCAL_VAR_DECL %a") && irContains(ir, "LOCAL_VAR_DECL %b") &&
...
void testIR_shortCircuitNested() {
  ...
  comp->globalDecls.push_back(makeFuncDef(
      Type::INT, "main", {},
      makeBlock({makeReturn(makeBinary(BinOp::AND, makeBinary(BinOp::OR, makeInt(1), makeInt(0)),
...
void testIR_shortCircuitWithCompare() {
  ...
  comp->globalDecls.push_back(makeFuncDef(
      Type::INT, "main", {},
      makeBlock({makeReturn(makeBinary(BinOp::AND, makeBinary(BinOp::GT, makeInt(3), makeInt(2)),
...
void testIR_globalVar() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {}, makeBlock({makeReturn(makeVar("g_x"))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_globalConst() {
  ...
  comp->globalDecls.push_back(
      makeFuncDef(Type::INT, "main", {}, makeBlock({makeReturn(makeVar("MAX"))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
  auto ir = irGen.generate(*comp);
...
void testIR_emptyExprStmt() {
  ...
  auto comp = makeCompUnit();
  comp->globalDecls.push_back(makeFuncDef(Type::INT, "main", {},
                                          makeBlock({makeExprStmt(nullptr), // 空语句 ;
                                                     makeReturn(makeInt(0))})));
  SemanticAnalyzer sema;
  sema.analyze(*comp);
  IRGenerator irGen(sema.getSymbolTable());
