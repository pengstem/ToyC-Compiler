#!/usr/bin/env python3
"""Verify a single-call helper is specialized with its constant arguments."""
...
def main() -> int:
    ...
    if re.search(r"(?m)^\s*call\s+evolve\b", main_body):
        raise RuntimeError("single-call helper was not inlined")
    if re.search(r"(?m)^L\d+:$", main_body):
        raise RuntimeError("constant-bound modular loop survived specialization")
    return 0
